...
安装固定版本依赖。如果数据库还没有 `source` 和 `external_id` 导入字段，先执行第一条迁移；启动更新后的 `job-service` 前必须执行第二条薪资迁移。薪资迁移可重复执行，不会删除已有数据：
...
Get-Content .\sql\2026-07-15-add-salary-text.sql -Raw | mysql --default-character-set=utf8mb4 -uroot -proot
```
